...
class Interacts:
    def __init__(self, interact, location, keyword, item, exits, message_attack, message_true, message_action, state):
    ...
    with open(filename, "r") as file:
        ...
        for interact, details in data.items():
            interacts[interact] = Interacts(
                interact=interact,
                # rooms.interacts=details["interact"], #let's sort this out later // no let's not
...
def main():
    ...
    while True:
        ...
        #print (player.move_history[last_move])
...
# This block needs sorting out
        if player.turns_in_room > 0:       # gives a chance for the player to see if there is something dangerous, but not take a second move in the room
            ...
            if player.current_room in rooms[player.current_room].interacts:   #if the current room has an interact in it
                print("bonza")
       #         if interacts[interact].message_attack:    #and if the interact has an attack message

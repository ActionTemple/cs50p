...
# 3/9/25 Can't kill an interact twice now
# 3/9/25 Implemented moves_possible condition which stops the player from making moves other than kill when there is an enemy in the same room
# 4/9/25 Fixed a bug which caused the moves_possible flag to be incorrectly set to False - causing an infinite loop
# 4/9/25 After an interact has been killed, he can now be searched
# 5/9/25 text_wrapper function to make sure all extra print lines are wrapped neatly to 80 columns without cutting words
# 5/9/25 Limited number of items in inventory to 2. (Also going to implement a rucksack to expand number to 8 or 10)
...
class Interacts:
    def __init__(self, interact, location, keyword, item, exits, pockets, talisman, message_talisman, message_attack, message_true, message_action, state=False):
...
def commands(b):
    ...
    action = re.search(r'^(get|drop|examine|search)()?(.+)?$', b)
